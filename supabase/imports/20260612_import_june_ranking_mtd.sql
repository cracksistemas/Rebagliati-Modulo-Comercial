...
expanded as (
  ...
  union all
  select executive_name, 'CM'::text as product_code, cm_qty as quantity, 2::numeric as unit_amount from source
  union all
  select executive_name, 'D'::text as product_code, d_qty as quantity, 6::numeric as unit_amount from source
),
admin_profile as (
  select id
  ...
  order by
    case
      when role = 'admin_sistema' then 0
      when role in ('gerencia', 'jefe_ventas') then 1
      else 2
    end,
    created_at asc
  limit 1
...
resolved as (
  select
    ...
    coalesce(tm.team_id, fallback_team.id) as team_id,
    pt.id as product_type_id,
    admin_profile.id as user_id
  from expanded
  join public.executives e
    on lower(replace(trim(e.full_name), '.', '')) = lower(replace(trim(expanded.executive_name), '.', ''))
  join public.product_types pt
    on pt.code = expanded.product_code
  left join lateral (
    select team_id
    from public.team_members
    where executive_id = e.id and active = true
...
select
  ...
  quantity * unit_amount,
  'No especificado',
  'Importacion ranking junio 2026',
  'validada',
